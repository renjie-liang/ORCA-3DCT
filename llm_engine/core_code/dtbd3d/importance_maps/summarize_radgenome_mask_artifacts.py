#!/usr/bin/env python
"""Summarize completed RadGenome mask artifact shards."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--artifact-dir",
        default="./data/organ_masks/radgenome_8x8x8",
    )
    parser.add_argument("--splits", default="train,valid")
    parser.add_argument("--expected-num-shards", type=int, default=64)
...
def main() -> int:
    ...
    all_status = Counter(row.get("status", "") for row in rows)
    summary["status_counts"] = dict(sorted(all_status.items()))
    ...
    for split in splits:
        ...
        status = Counter(row.get("status", "") for row in rows_for_split)
        n_masks = [
            int(row["n_masks"])
