...
from shared.models.ctclip.mlm import MLM
from shared.models.ctclip.visual_ssl import SimSiam, SimCLR
...
class CTCLIP(nn.Module):
    def __init__(
            ...
            *,
            ...
            checkpoint_during_training = False,
            projection_dropout = 0.0,  # Dropout rate for projection layers (anti-overfitting)
            **kwargs
    ):
        super().__init__()
        ...
        # Add dropout for regularization (anti-overfitting)
        if projection_dropout > 0:
            self.to_text_latent = nn.Sequential(
                nn.Linear(dim_text, dim_latent, bias = False),
        ...
        if downsample_image_embeds:
            ...
            if projection_dropout > 0:
                layers.append(nn.Dropout(projection_dropout))
            self.to_visual_latent = nn.Sequential(*layers)
        else:
            if projection_dropout > 0:
    ...
    def forward(
            ...
            device,
            return_loss = False,
            return_encodings = False,  # Deprecated: kept for compatibility
            return_latents = False,    # Deprecated: kept for compatibility
        ...
        """
        Unified forward pass - always returns dict with all outputs

        Returns:
            {
                'loss': Tensor or None,              # Contrastive loss (only if return_loss=True)
                'image_embeddings': Tensor,          # Image encoder output
                'text_embeddings': Tensor,           # Text encoder output
                'image_latents': Tensor,             # L2-normalized image latents
                'text_latents': Tensor,              # L2-normalized text latents
                'logit_scale': Tensor,               # Temperature scaling factor
                'similarity': Tensor or None,        # Similarity scores (only if return_loss=False)
            }
        """
        # Handle both dict and object format for text
        if isinstance(text, dict):
            text_input_ids = text['input_ids']
            text_attention_mask = text.get('attention_mask', None)
        else:
            text_input_ids = text.input_ids
            text_attention_mask = text.attention_mask
        
        b, device = text_input_ids.shape[0], device

        # derive text mask
        text_mask = text_attention_mask if text_attention_mask is not None else (text_input_ids != self.text_pad_id)
        ...
        if return_loss:
            ...
            #print("------------")
            text_ssl_loss = self.mlm(text_input_ids, attention_mask = text_attention_mask) if self.use_mlm else 0
        ...
        if exists(aug_text):
            ...
            # Handle both dict and tensor format for aug_text
            if isinstance(aug_text[0], dict):
                aug_text_input_ids = torch.cat([t['input_ids'] for t in aug_text], dim=0)
                aug_text_attention_mask = torch.cat([t.get('attention_mask', torch.ones_like(t['input_ids'])) for t in aug_text], dim=0)
            else:
                # Assume aug_text is a list of tensors (old format)
                aug_text_input_ids = torch.cat(aug_text, dim=0)
                aug_text_attention_mask = aug_text_input_ids != self.text_pad_id
            
            num_batch_texts = len(aug_text) + 1
            aug_text_mask = aug_text_input_ids != self.text_pad_id

            text_mask = torch.cat((text_mask, aug_text_mask), dim = 0)
            text_input_ids = torch.cat((text_input_ids, aug_text_input_ids), dim = 0)
            if text_attention_mask is not None:
                text_attention_mask = torch.cat((text_attention_mask, aug_text_attention_mask), dim = 0)
            else:
                text_attention_mask = aug_text_attention_mask
        ...
        text_args = (text_input_ids, text_attention_mask)

        if not self.text_encode_without_mask:
            text_args = (*text_args, text_mask)


        text_embeddings = self.text_transformer(text_input_ids, attention_mask = text_attention_mask)
        ...
        if self.text_causal_mask:
            eos_text_mask = (text_input_ids == self.text_eos_id)
        ...
        # Note: return_encodings deprecated - all encodings returned in dict

        global h_r, w_r, z_r
        h_r, w_r, z_r = enc_image.shape[1], enc_image.shape[2], enc_image.shape[3]
        enc_image_send = enc_image  # Original CTViT output (B, T, H, W, D)

        # CTViT already applied spatial pooling, just flatten
        # enc_image: (B, T, H, W, D) -> (B, T*H*W*D)
        # Use reshape instead of view to handle non-contiguous tensors after pooling
        enc_image = enc_image.reshape(enc_image.shape[0], -1)
        ...
        # Project image embeddings to latents
        image_latents = self.to_visual_latent(image_embeds)
        text_latents, image_latents = map(l2norm, (text_latents, image_latents))

        # All-Gather for multi-GPU training
        from shared.training.distributed import is_distributed, all_gather

        if is_distributed():
            text_latents = all_gather(text_latents)
            image_latents = all_gather(image_latents)
            b = text_latents.shape[0]  # Update to global batch size
        ...
        if self.extra_latent_projection:
            ...
            if is_distributed():
                text_latents_extra = all_gather(text_latents_extra)
                image_latents_extra = all_gather(image_latents_extra)

        # Note: return_latents deprecated - all latents returned in dict

        # Store original latents before multiview processing
        original_text_latents = text_latents.clone()
        ...
        # Calculate similarity scores (always compute for output dict)
        similarity = None
        if not return_loss:
            if self.use_all_token_embeds:
                ...
                similarity = einsum('b d, b i d -> b t i', *einsum_args) * temp
            else:
                einsum_args = (text_latents_extra, image_latents_extra) if self.extra_latent_projection and not text_to_image else (text_latents, image_latents)
                similarity = einsum('b d, b d -> b', *einsum_args) * temp
        ...
        # Return unified dict with all outputs
        return_dict = {
            'loss': loss,
            'image_embeddings': enc_image_send,  # Original CTViT output (B, T, H, W, D)
            'text_embeddings': enc_text,
            'image_latents': original_image_latents,
            'text_latents': original_text_latents,
            'logit_scale': temp,
    ...
    def encode_image(self, image: torch.Tensor) -> torch.Tensor:
        """Return L2-normalized image latents (no text needed)."""
        with torch.no_grad():
            enc_image = self.visual_transformer(image, return_encoded_tokens=True)
            # CTViT already applied spatial pooling, just flatten
            # Use reshape instead of view to handle non-contiguous tensors after pooling
            ...
            return F.normalize(image_latents, dim=-1)

    def encode_image_3d(self, image: torch.Tensor) -> torch.Tensor:
        """
        Return 3D embedding for LLM input (preserves 3D structure).

        Args:
            image: CT volume (B, 1, D, H, W)

        Returns:
            3D embedding (B, T', H', W', D') - CTViT output with spatial pooling

        Example:
            Input: (1, 1, 150, 480, 480) - CT volume
            Output: (1, 5, 8, 8, 512) - After CTViT + pooling
        """
        with torch.no_grad():
            enc_image = self.visual_transformer(image, return_encoded_tokens=True)
            # CTViT already applied spatial pooling
            return enc_image

    def encode_text(self, text) -> torch.Tensor:
        """Return L2-normalized text latents (no image needed)."""
        with torch.no_grad():
            # Encode text
            text_embeddings = self.text_transformer(text.input_ids, attention_mask=text.attention_mask)
            enc_text = text_embeddings[0]

            # Extract text embeds (CLS token)
            text_embeds = enc_text[:, 0, :]

            # Project to latents
            text_latents = self.to_text_latent(text_embeds)
            return F.normalize(text_latents, dim=-1)
