...
from shared.models.ctvit.attention import Attention, Transformer, ContinuousPositionBias
...
class CTViT(nn.Module):
    ...
    def encode(
        ...
        # Memory cleanup before temporal encoding (critical memory bottleneck)
        ...
        *_, h, w, _ = shape
        ...
        if return_encoded_tokens:
            if self.use_spatial_pooling:
                # Apply 3D average pooling: (B, T, H, W, D) -> (B, T', H', W', D)
                tokens = rearrange(tokens, 'b t h w d -> b d t h w')
                tokens = F.avg_pool3d(tokens, kernel_size=self.pooling_kernel, stride=self.pooling_stride)
