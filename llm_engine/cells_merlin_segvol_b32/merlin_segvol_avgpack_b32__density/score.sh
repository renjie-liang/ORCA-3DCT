#!/usr/bin/env bash
# merlin_segvol_avgpack_b32/density score every per-epoch eval -> best.json (CPU, after s1)
set -euo pipefail
DEP="${1:-}"
sbatch --parsable --time=00:40:00 --partition=hpg-default --account=anon --ntasks=1 --cpus-per-task=2 --mem=16gb \
  --job-name=msc_segvol_avgpack_b32_density \
  ...
  --wrap="export MAMBA_EXE=/home/anon/micromamba MAMBA_ROOT_PREFIX=/blue/anon/anon/micromamba; \
    ...
    python ./llm_engine/score_sweep.py --run_dir ./results_llm/merlin_segvol_b32/vqa_single_merlin_segvol_avgpack_b32__density/vqa_single_merlin_segvol_avgpack_b32__density__s1 --gold ./data/vqa_merlin_v1/per_family/valid_density_merlin.json"
